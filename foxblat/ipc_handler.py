...
class IpcHandler(EventDispatcher):
    """
    Handles IPC via TCP socket to allow external programs to control foxblat.
    ...
    def _start_tcp_socket(self) -> bool:
        ...
        try:
            ...
            if e.errno == 98:  # Address already in use
                print(f"[IPC] Port {self._tcp_port} already in use (another foxblat instance running?)")
            else:
