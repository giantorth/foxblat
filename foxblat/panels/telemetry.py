...
class TelemetrySettings(SettingsPanel):
    ...
    def prepare_ui(self):
        ...
        self._status_label = FoxblatLabelRow("Status", "SimAPI daemon connection", "Checking...")
        self._add_row(self._status_label)

        self._sim_label = FoxblatLabelRow("Simulator", "Current simulation", "None")
        self._add_row(self._sim_label)

        self._car_label = FoxblatLabelRow("Car", "Current vehicle", "None")
        self._add_row(self._car_label)

        self.add_preferences_group("Live Telemetry")
        self._rpm_raw_label = FoxblatLabelRow("Raw RPM", "Current / Max (effective) / Idle", "0 / 0 (0) / 0")
        self._add_row(self._rpm_raw_label)

        self._rpm_level = FoxblatLevelRow("RPM %", max_value=100)
        self._rpm_level.set_bar_width(400)
        self._add_row(self._rpm_level)

        self._gear_label = FoxblatLabelRow("Gear", "", "N")
        ...
        self._add_row(FoxblatButtonRow("Reset RPM Calibration", "Reset", "Rev to max RPM to recalibrate"))
        ...
        self._debug_switch = FoxblatSwitchRow("Enable Debug View", "Show raw telemetry data below")
